...
class ShuffleNet2(nn.Module):
  def __init__(self, num_classes=2, input_size=val_dataset.__getitem__(5)[0].shape[-1], net_type=0.5, RGB=False):
    ...
    if RGB:
        ...
        self.conv1 = nn.Conv2d(1, self.out_channels[1], 3, 2, 1)
  ...
  def forward(self, x):
    ...
    x = self.conv5(x)
    x = self.g_avg_pool(x)
    x = self.flatten(x)#.view(-1, self.out_channels[-1])
    x = self.fc(x)
    return x

device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
model = ShuffleNet2(RGB=True).to(device)
